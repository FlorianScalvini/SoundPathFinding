...
int LavCamera::xioctl(int fd, int request, void *arg)
{
    return ioctl (fd, request, arg);
}

LavCamera::LavCamera(int IDCam, int width, int height)
{
	...
	lavLog::LAVLOG("init");
	if(print_caps(fd)) {
		lavLog::LAVLOG("pb init");
	}

	lavLog::LAVLOG("mmap");
	if(init(fd)) {
		lavLog::LAVLOG("pb init_mmap");
	}

}


int LavCamera::print_caps(int fd)
{
	...
	{
		lavLog::LAVLOG("pb Setting Pixel Format");
		return 1;
	}
...
void LavCamera::convertToOpenCv() {
...
void LavCamera::closeAcquisition() {

    lavLog::LAVLOG("Close camera acquisition");
