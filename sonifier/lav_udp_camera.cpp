...
lavUdpCamera::lavUdpCamera(const std::string& addr, int port)
    : f_port(port)
    , f_addr(addr)
{
    lavLog::LAVLOG("lavUdpCamera::lavUdpCamera");
    ...
    {
        lavLog::LAVLOG("invalid address or port for UDP socket: ");
        lavLog::LAVLOG(addr.c_str());
    ...
    if(f_socket == -1)
    {
        ...
        lavLog::LAVLOG(("could not create UDP socket for: \"" + addr + ":" + decimal_port + "\"").c_str());
    ...
    if(r != 0)
    {
        freeaddrinfo(f_addrinfo);
        close(f_socket);
        lavLog::LAVLOG(("could not bind UDP socket with: \"" + addr + ":" + decimal_port + "\"").c_str());
        //throw udp_client_server_runtime_error(("could not bind UDP socket with: \"" + addr + ":" + decimal_port + "\"").c_str());
    }
}


lavUdpCamera::~lavUdpCamera()
{
    freeaddrinfo(f_addrinfo);
    close(f_socket);
}
...
int lavUdpCamera::get_socket() const
{
    return f_socket;
}


int lavUdpCamera::get_port() const
{
    return f_port;
}


std::string lavUdpCamera::get_addr() const
{
    return f_addr;
}


int lavUdpCamera::recv(char *msg, size_t max_size)
{
    return ::recv(f_socket, msg, max_size, 0);
}


int lavUdpCamera::timed_recv(char *msg, size_t max_size, int max_wait_ms)
{
    fd_set s;
    FD_ZERO(&s);
    FD_SET(f_socket, &s);
...
/*
cv::Mat lavUdpCamera::getNextFrame()
{
    lavLog::LAVLOG("lavUdpCamera::getNextFrame/n");
