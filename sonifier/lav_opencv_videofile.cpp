...
lavOpencvVideoFile::lavOpencvVideoFile(char* filename, int fps)
{
...
cv::Mat lavOpencvVideoFile::getNextFrame()
{
    //lavLog::LAVLOG("lavOpencvVideoFile::getNextFrame");
    ...
    if (_tmpMat.size().width !=0) {
        ...
        if ((_tmpMat.size().width != FRAME_WIDTH_SONIFIED) || (_tmpMat.size().height != FRAME_HEIGHT_SONIFIED)) {
            ...
            //lavLog::LAVLOG("lavOpencvVideoFile::getNextFrame2");
        }
        ...
        if (!_isFirstFrame) {
            ...
            //lavLog::LAVLOG("lavOpencvVideoFile::getNextFrame sleep", _maxUsSleepingPeriod-_usTime);
            ...
            //lavLog::LAVLOG("lavOpencvVideoFile::getNextFrame 2", _usTime);
...
void lavOpencvVideoFile::release()
{
    _capture->release();
}
