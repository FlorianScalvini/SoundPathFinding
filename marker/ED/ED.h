...
EdgeMap *DetectEdgesByEDV(unsigned char *srcImg, int width, int height, GradientOperator op=PREWITT_OPERATOR, int GRADIENT_THRESH=20, int ANCHOR_THRESH=0, double smoothingSigma=1.0);

/// Detect Edges by Edge Drawing Parameter Free (EDPF). smoothingSigma must be >= 1.0
EdgeMap *DetectEdgesByEDPF(unsigned char *srcImg, int width, int height, double smoothingSigma=1.0);
...
/// Use cvCanny and return the result as an EdgeMap. smoothingSigma must be >= 1.0. Use ED2 for linking
EdgeMap *CannyEDetectEdgesByCannyS2(unsigned char *srcImg, int width, int height, int cannyLowThresh, int cannyHighThresh, int sobelKernelApertureSize=3, double smoothingSigma=1.0);

/// Use cvCanny and return the result as an EdgeMap. smoothingSigma must be >= 1.0. Validate the edge segments after detection
...
EdgeMap *DetectEdgesByED(unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, int width, int height, 
                         int GRADIENT_THRESH=20, int ANCHOR_THRESH=0, double smoothingSigma=1.0);
...
EdgeMap *DetectEdgesByEDPF(unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, int width, int height, double smoothingSigma=1.0);
...
EdgeMap *DetectContourEdgeMapByED2(unsigned char *contourImg, int width, int height, double smoothingSigma = 0.675);
...
EdgeMap *DetectContourEdgeMapByED3(unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, unsigned char *contourImg, int width, int height, 
                                   int GRADIENT_THRESH, int ANCHOR_THRESH, double maxSigma=2.75);

/// Detects edges by changing the size of the gradient operator depending on "smoothingSigma" parameter and sets GRADIENT_THRESH accordingly
EdgeMap *DetectEdgesByED10(unsigned char *srcImg, int width, int height, double smoothingSigma);
...
EdgeMap *DetectEdgesByED11(unsigned char *srcImg, int width, int height, double smoothingSigma);
