...
///
static void ComputeAnchorPoints(short *gradImg, unsigned char *dirImg, EdgeMap *map, int GRADIENT_THRESH, int ANCHOR_THRESH, int SCAN_INTERVAL){
  int width = map->width;
...
///
static void ComputeAnchorPoints4Dirs(short *gradImg, unsigned char *dirImg, EdgeMap *map, int GRADIENT_THRESH, int ANCHOR_THRESH, int SCAN_INTERVAL){
  int width = map->width;
  int height = map->height;
...
/// 
static int *SortAnchorsByGradValue(short *gradImg, EdgeMap *map, int *pNoAnchors){ 
  int width = map->width;
  int height = map->height;
...
///
static void JoinAnchorPoints(short *gradImg, unsigned char *dirImg, EdgeMap *map, int GRADIENT_THRESH, int MIN_PATH_LEN){
  int width = map->width;
  int height = map->height;
...
///
//static 
void JoinAnchorPointsUsingSortedAnchors4Dirs(short *gradImg, unsigned char *dirImg, EdgeMap *map, int GRADIENT_THRESH, int MIN_PATH_LEN){
  int width = map->width;
  int height = map->height;
      ...
      while (top >= 0){
        ...
        if (dir == LEFT){
          ...
          while (dirImg[r*width+c] == EDGE_45){
            ...
            //
            // cleanup side pixels
            if (edgeImg[(r-1)*width+c-1] == ANCHOR_PIXEL) edgeImg[(r-1)*width+c-1] = 0;
            if (edgeImg[(r+1)*width+c+1] == ANCHOR_PIXEL) edgeImg[(r+1)*width+c+1] = 0;
            if (edgeImg[r*width+c-1] == ANCHOR_PIXEL) edgeImg[r*width+c-1] = 0;
            if (edgeImg[(r+1)*width+c] == ANCHOR_PIXEL) edgeImg[(r+1)*width+c] = 0;

            // Look if there is an edge pixel in the neighbors
            if (edgeImg[(r-1)*width+c+1] >= ANCHOR_PIXEL){r--; c++;}
            else if (edgeImg[(r-1)*width+c] >= ANCHOR_PIXEL){r--;}
            else if (edgeImg[r*width+c+1] >= ANCHOR_PIXEL){c++;}
            else {
...
/// 
EdgeMap *DoDetectEdgesByED(short *gradImg, unsigned char *dirImg, int width, int height, int GRADIENT_THRESH, int ANCHOR_THRESH){
...
/// Detect edges by edge drawing method
/// 
EdgeMap *DoDetectEdgesByED4Dirs(short *gradImg, unsigned char *dirImg, int width, int height, int GRADIENT_THRESH, int ANCHOR_THRESH){
  // Check parameters for sanity
  if (GRADIENT_THRESH < 1) GRADIENT_THRESH = 1;
  if (ANCHOR_THRESH < 0) ANCHOR_THRESH = 0;
  int SCAN_INTERVAL = 1; //2
  ...
  /*------------ COMPUTE ANCHORS -------------------*/
  ComputeAnchorPoints4Dirs(gradImg, dirImg, map, GRADIENT_THRESH, ANCHOR_THRESH, SCAN_INTERVAL);

  /*------------ JOIN ANCHORS -------------------*/
  JoinAnchorPointsUsingSortedAnchors4Dirs(gradImg, dirImg, map, GRADIENT_THRESH, MIN_PATH_LEN);

  return map;
} //DoDetectEdgesByED4Dirs


/**************************** ALTERNATIVE ED THAT DOES NOT USE EDGE DIRECTIONS FOR LINKING ***********************/
#if 0
///-------------------------- Original EDWalk ------------------------------
///-------------------------------------------------------------------------
  ...
  while (1){
    ...
    if        (dir == UP_LEFT){
      ...
      if (nextDir < 0){
        ...
        if (gradImg[(r-1)*width+c+1] > gradImg[(r+1)*width+c-1]){
          grad = gradImg[(r-1)*width+c+1];
          if (grad > maxGrad){maxGrad = grad; nr = r-1; nc = c+1; nextDir = UP_RIGHT;}
        } else {
          grad = gradImg[(r+1)*width+c-1];
...
///------------------------------------------------------------------------------------------------
/// Alternative anchor linking algorithm that works only by walking over the maximas of the gradImg. 
/// Does not use edge directions
///
void JoinAnchorPointsUsingSortedAnchors(short *gradImg, EdgeMap *map, int GRADIENT_THRESH, int MIN_PATH_LEN){
  int width = map->width;
  int height = map->height;
  ...
  for (int k=noAnchors-1; k>=0; k--){
    ...
    if (gradImg[(i-1)*width+j] > maxGrad){maxGrad = gradImg[(i-1)*width+j]; dir1 = UP; dir2 = DOWN;}
    if (gradImg[(i+1)*width+j] > maxGrad){maxGrad = gradImg[(i+1)*width+j]; dir1 = DOWN; dir2 = UP;}
    if (gradImg[i*width+j+1] > maxGrad){maxGrad = gradImg[i*width+j+1]; dir1 = RIGHT; dir2 = LEFT;}

    if (gradImg[(i-1)*width+j-1] > maxGrad){maxGrad = gradImg[(i-1)*width+j-1]; dir1 = UP_LEFT; dir2 = DOWN_RIGHT;}
    if (gradImg[(i-1)*width+j+1] > maxGrad){maxGrad = gradImg[(i-1)*width+j+1]; dir1 = UP_RIGHT; dir2 = DOWN_LEFT;}

    if (gradImg[(i+1)*width+j-1] > maxGrad){maxGrad = gradImg[(i+1)*width+j-1]; dir1 = DOWN_LEFT; dir2 = UP_RIGHT;}
    if (gradImg[(i+1)*width+j+1] > maxGrad){maxGrad = gradImg[(i+1)*width+j+1]; dir1 = DOWN_RIGHT; dir2 = UP_LEFT;}

    int len1 = EDWalk(gradImg, map, GRADIENT_THRESH, i, j, dir1, tmpPixels);
    int len2 = EDWalk(gradImg, map, GRADIENT_THRESH, i, j, dir2, tmpPixels+len1);
...
EdgeMap *DoDetectEdgesByED(short *gradImg, int width, int height, int GRADIENT_THRESH){
  ...
  for (int i=1; i<height-1; i++){
    for (int j=1; j<width-1; j++){
      ...
      // Assume that a pixel is an anchor only if it is a local maxima in 8 neighborhood
  ...
  /*------------ JOIN ANCHORS -------------------*/
  JoinAnchorPointsUsingSortedAnchors(gradImg, map, GRADIENT_THRESH, MIN_PATH_LEN);
