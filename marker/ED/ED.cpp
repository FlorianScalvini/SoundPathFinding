...
EdgeMap *DetectEdgesByCannySR(unsigned char *srcImg, int width, int height, int cannyLowThresh, int cannyHighThresh, int sobelKernelApertureSize, double smoothingSigma){
  ...
  auto *iplImg = new cv::Mat(cv::Size(width, height), CV_8UC1, srcImg);
  auto  *edgeImg = new cv::Mat(cv::Size(width, height), CV_8UC1);

    if (smoothingSigma <= 1.0)
        cv::GaussianBlur(*iplImg, *iplImg, cv::Size(5,5), 0, 0);
    else
        cv::GaussianBlur(*iplImg, *iplImg, cv::Size(7, 7), smoothingSigma);
  // Copy the smooth image
  auto *smoothImg = new unsigned char[width*height];
  for (int i=0; i<height; i++){
      for (int j=0; i<width; i++){
          smoothImg[i*width+j] = iplImg->at<unsigned char>(i,j);
      }
  }
  ...
  // Detect edges by cvCanny
    if (sobelKernelApertureSize != 3 && sobelKernelApertureSize != 5 && sobelKernelApertureSize != 7) sobelKernelApertureSize = 3;
        cv::Canny(*iplImg, *edgeImg, cannyLowThresh, cannyHighThresh, sobelKernelApertureSize);
  ...
  memset(map->edgeImg, 0, width*height);
    ...
    for(int i=0; i < height; i++)
    {
        ...
        {
            if(edgeImg->at<unsigned char>(i, j))
                map->edgeImg[i*width+j] = 254;
        }
    }

  auto *dirImg = new unsigned char[width*height];
  auto *gradImg = new short[width*height];


    cv::GaussianBlur(*edgeImg, *edgeImg, cv::Size(5,5), 0, 0);
  ...
  memset(gradImg, 0, sizeof(short)*width*height);
    for (int i=1; i<height-1; i++){
        for (int j=1; j<width-1; j++){
            if (edgeImg->at<unsigned char>(i, j) < 32) continue;
...
//  cvSmooth(edgeImg, edgeImg, CV_GAUSSIAN, 5, 5);
    cv::GaussianBlur(*edgeImg, *edgeImg, cv::Size(7,7), 0.5, 0.5);
    ...
    for(int i=0; i < height; i++)
    {
        for(int j=0; j < width; j++)
        {
            if(gradImg[i*width+j])
                continue;
            if(edgeImg->at<unsigned char>(i, j))
                gradImg[i*width+j] = edgeImg->at<unsigned char>(i, j);
        }
    }
